...
$APPLICATION->SetTitle('Создание копии заявки на подбор');
...
// ===== Предзаполнение формы по исходной заявке =====
...
if ($sourceRequestId > 0) {
    $srcRs = CIBlockElement::GetList(
        [],
        ['IBLOCK_ID' => IBLOCK_RECRUIT, 'ID' => $sourceRequestId],
        false,
        false,
        ['ID', 'NAME', 'PROPERTY_JSON', 'PROPERTY_3036']
    );
    ...
    $prefillError = 'Не указан id исходной заявки в URL (ожидается create_request_copy.php?id=...).';
}
if ($_SERVER['REQUEST_METHOD'] === 'POST' && check_bitrix_sessid() && ($_POST['action'] ?? '') === 'save') {
    ...
    $props = [
        ...
        'OBORUDOVANIE_DLYA_RABOTY_STROKA'               => $equipName,
...
<div class="container my-4">
  <div class="d-flex align-items-center mb-3">
    ...
    <?php if ($sourceRequestId > 0): ?>
      <span class="ml-2 badge badge-info">Источник: #<?= $sourceRequestId ?></span>
    <?php endif; ?>
  </div>
  <?php if ($prefillError): ?>
          ...
          <div class="form-group col-md-6">
            ...
            $APPLICATION->IncludeComponent(
                ...
                [
                    ...
                    'INPUT_VALUE'         => ($prefillEmployeeId > 0 ? [$prefillEmployeeId] : []),
                    'MULTIPLE'            => 'N',
...
BX.ready(function(){
  BX.UI.Hint.init();
  function prefillByName(name, value) {
    ...
    if ($field.attr('type') === 'checkbox' || $field.attr('type') === 'radio') {
      if (Array.isArray(value)) {
        $field.each(function () {
          $(this).prop('checked', value.map(String).indexOf(String($(this).val())) !== -1);
        });
      } else {
        $field.each(function () {
  ...
  function applyPrefill() {
    ...
    var map = [
      'legal','dep0','position','position_custom','directorate','employee_name','stake','reason',
      'reason_expand','maternity_date','maternity_fio','replacement_date','replacement_fio',
      'transfer_date','transfer_fio_where','duties_original','duties','gender','education','lang',
      'experience','softskills','softwares','requirements_extra','driver_license','schedule','start_time',
      'format','office','business_trips','trip_duration','contract','equipment','equipment_comment',
      'confidential','internal_candidate','internal_departments','from_positions'
    ];
    map.forEach(function(name){
      if (Object.prototype.hasOwnProperty.call(prefillData, name)) {
        prefillByName(name, prefillData[name]);
      }
    });

    if (Object.prototype.hasOwnProperty.call(prefillData, 'furniture')) {
      prefillByName('furniture[]', prefillData.furniture);
    }

    var employeeId = prefillData.employee_id;
    if (Array.isArray(employeeId) && employeeId.length > 0) {
      employeeId = employeeId[0];
    }
    if (employeeId) {
      prefillByName('employee_id', employeeId);
    }
    if (prefillData.employee_name) {
      prefillByName('employee_name', prefillData.employee_name);
    }

    if (prefillData.position_custom) {
