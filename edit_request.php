 ...
 * Версия: v2.0.0 (2026-03-30)
 *
 * v2.0.0:
 * - Добавлены проверки доступа для ролей: C&B, рекрутер заявки, руководители отдела подбора
 * - Добавлен параметр par_Changes_type при запуске БП (cb / recruiter)
 * - Для рекрутера и руководителя подбора ограничен набор редактируемых полей
 */
...
const ROLE_LABEL_RECRUITER = 'Рекрутер';
const ROLE_LABEL_RECRUITMENT_HEAD = 'Руководитель отдела подбора';
 ...
 * 3A) ACCESS HELPERS
 * =======================================================*/
function getBpGlobalVarUsers($varId) {
    $users = [];
    try {
        $conn = \Bitrix\Main\Application::getConnection();
        $varIdSql = $conn->getSqlHelper()->forSql((string)$varId);
        $row = $conn->query("
...
$FIELDS = [
    ...
    ["CODE" => "OBORUDOVANIE_DLYA_RABOTY_TEKST", "NAME" => "Доп. требования к оборудованию", "EDITABLE" => false],
    ["CODE" => "NEOBKHODIMAYA_MEBEL", "NAME" => "Необходимая мебель", "EDITABLE" => false],
...
/* =========================================================
 * 3B) ACCESS CHECK (C&B / RECRUITER / RECRUITMENT HEAD)
 * =======================================================*/
$currentUserId = (int)$USER->GetID();
$currentUserCode = 'user_' . $currentUserId;

$cbUsers = getBpGlobalVarUsers('Variable1722502594854');
$recruitmentHeadUsers = getBpGlobalVarUsers('Variable1722503621093');
$requestRecruiterId = (int)normPropValue($curProps['REKRUTER'] ?? 0);

$isCbManager = in_array($currentUserCode, $cbUsers, true);
$isRecruitmentHead = in_array($currentUserCode, $recruitmentHeadUsers, true);
$isRequestRecruiter = ($requestRecruiterId > 0 && $requestRecruiterId === $currentUserId);

$editorRole = '';
$changesType = '';
if ($isCbManager) {
    $editorRole = ROLE_LABEL_CB;
    $changesType = 'cb';
} elseif ($isRequestRecruiter) {
    $editorRole = ROLE_LABEL_RECRUITER;
    $changesType = 'recruiter';
} elseif ($isRecruitmentHead) {
    $editorRole = ROLE_LABEL_RECRUITMENT_HEAD;
    $changesType = 'recruiter';
}

if ($editorRole === '') {
    ShowError('У вас нет прав на редактирование этой заявки');
    require($_SERVER['DOCUMENT_ROOT'].'/bitrix/footer.php');
    return;
}

$recruiterEditableCodes = [
...
function isFieldEditableForRole($code, $baseEditable, $isCbManager, $recruiterEditableCodes) {
    if ($isCbManager) {
        return (bool)$baseEditable;
    }
    return in_array((string)$code, $recruiterEditableCodes, true);
}
...
if ($request->isPost() && check_bitrix_sessid()) {
    ...
    foreach ($FIELDS as $f) {
        if (!isFieldEditableForRole($f['CODE'], !empty($f['EDITABLE']), $isCbManager, $recruiterEditableCodes)) continue;
    ...
    if (empty($updates)) {
        ...
        $historyBlock = "[{$stamp}] Изменения (" . $editorRole . ", {$who}):\n- " . implode("\n- ", $historyChanged);
...
$bpParams = [
            ...
            'par_Changes_type' => (string)$changesType,
        ];
...
<form method="post" class="ui-form" style="max-width: 1100px;">
  ...
  foreach ($GROUP_ORDER as $groupTitle) {
      ...
      foreach ($byGroup[$groupTitle] as $f) {
          ...
          $meta = $metaMap[$code] ?? null;
          $val  = $curProps[$code] ?? '';
          $editable = isFieldEditableForRole($code, !empty($f['EDITABLE']), $isCbManager, $recruiterEditableCodes);
          echo renderInput($code, (string)$f['NAME'], $editable, $meta, $val, $REFERENCE_IBLOCK_BY_CODE);
      }
